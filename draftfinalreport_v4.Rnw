...
vacc_reqd_auckland   <- min(floor((vacc_dhb %>% filter(DHB == "Auckland" | DHB == "CountiesManakau" | DHB == "Waitemata") %>% select(Vaccination)) / 1000)) * 1000
vacc_reqd_count      <- totals$Vaccination
naive_percent        <- round(totals$Naïve / totals$Population * 100)
break_even_vacc_cost <- 259 # vaccine costs such that benefits == cost (from .xlsx)
@
