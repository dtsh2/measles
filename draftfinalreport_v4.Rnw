...
costtable20[,2:10] <- round(costtable20[,2:10])
...
costtable50[,2:10] <- round(costtable50[,2:10])
...
\begin{sidewaystable}\small
<<results=tex,echo=FALSE>>=
colnames(costtable74)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C")
costtable74[,2:10] <- round(costtable74[,2:10])
