...
\PassOptionsToPackage{draft}{graphicx} % draft without figures
...
### NOTE: DON'T CHANGE THE ORDER OF THESE INCLUDES
library(MASS)
library(plyr)
library(reshape)
library(raster)
...
vacc_dhb <- read.csv("tables/dhb_vacc.csv")
totals   <- vacc_dhb[nrow(vacc_dhb), ,drop=FALSE]
vacc_reqd_percent    <- totals$PC * 100
vacc_reqd_dhb_min    <- min(vacc_dhb$PC) * 100
vacc_reqd_dhb_max    <- max(vacc_dhb$PC) * 100
vacc_reqd_auckland   <- format(min(floor((vacc_dhb %>% filter(DHB == "Auckland" | DHB == "CountiesManakau" | DHB == "Waitemata") %>% select(Vaccination)) / 1000)) * 1000, big.mark=",")
vacc_reqd_count      <- format(totals$Vaccination, big.mark=",")
naive_percent        <- round(totals$Naive / totals$Population * 100)
break_even_vacc_cost <- format(74, big.mark=",") # vaccine costs such that benefits == cost (from .xlsx)
break_even_vacc_cost50 <- sprintf("%0.2f", 35.70) # vaccine costs such that benefits == cost (from .xlsx)
break_even_vacc_cost_no <- format(1877, big.mark=",")  # vaccine costs such that benefits == cost (from .xlsx)
@
...
\item People of European ethnicity make up the majority of cases. 
\item The regression analyses suggest that age is a particularly strong risk factor for measles.  Increased wealth (NZDep 1--3) was a risk factor and Pacific people are at greater risk on a per capita basis. However, this was driven by 0--2 year old Pacific children, as 2--24 year old Pacific people are less at risk than European and Maori, as are 5--17 year old Asian children.
\item Of the \Sexpr{naive_percent}\% of the overall population of New Zealand that are currently considered to be immunologically na\"{\i}ve to measles, it was estimated that approximately \Sexpr{vacc_reqd_percent}\% of those would require additional vaccination on top of current childhood vaccination rates to ensure measles does not persist and outbreaks do not occur, leading to an additional \Sexpr{vacc_reqd_count} effective vaccinations nationally.
\item New Zealand is at risk of frequent measles importation due to travel and endemic measles elsewhere in the globe. Peak overall overseas travel rates, and thus presumably risk from measles importation, are in December. However, otherwise New Zealander and non--New Zealander travel seasonality differs and is out of phase, with peak travel for New Zealanders during winter, and non--New Zealanders during summer.
\item Analyses of outbreak data suggest that measles basic reproduction number values ($R_v$, the number of secondary infections in a partially immunised population) often include or exceed one, including 2013--2014 outbreaks that were ongoing at the time of analysis. As analysed from data until 12 June 2014, the last outbreak to end in 2014 had an $R_v$ well above 1 (mean estimate 3.66, range 3.4--3.92). The range of estimates for $R_v$ between 2009--2014 was 0.18--3.92. These analyses suggest improved vaccination is a requisite to prevent measles persisting in the population and that large outbreaks could occur.
\item The cost of the first 187 measles cases in 2014 is estimated to be approximately \$1,041,186 due to earnings lost for cases and contacts, case management and hospitalisation costs.
...
\item The benefit--cost (B/C) ratio analyses suggest additional vaccination is beneficial financially (B/C $>$1). Vaccination-related costs are required to exceed approximately \$\Sexpr{break_even_vacc_cost} to \$\Sexpr{break_even_vacc_cost_no} per person currently na\"{\i}ve immunized through vaccination before the costs exceed the benefits for all DHBs together, depending on the scenario modelled. In the former case, control measures, their costs and measles outbreak sizes are assumed to remain the same as 2009 onward, whereas in the latter example no control measures are implemented and the outbreak size is based on epidemiological theory in the absence of these control measures.
...
\item Measles introductions to New Zealand with a median outbreak size of 2 cases were predicted by simulation models, despite additional vaccination to ensure $R_v$ is < 1. Thus, typically individual cases would be expected on average to infect a single secondary cases. However, the mean outbreak size of 61 cases was predicted, because outbreak sizes of tens, hundreds, and very rarely up to many thousands of cases are predicted among the 1000 simulations following importation, despite $R_v$ being one and the outbreak predicted to die out. Thus, increased vaccination beyond the \Sexpr{vacc_reqd_percent}\% of the currently \Sexpr{naive_percent}\% na\"{\i}ve population required may be useful to prevent these rare but costly events.
\end{itemize}
...
resan<-xtable(anova(model3, test="F")[-1,6, drop=FALSE])
xtb<-xtable(summary(model3)$coefficients[,-3])
...
round((sum(NaiveByYear$Naive[10:29+1])/sum(NaiveByYear$Naive)),2),
round((sum(NaiveByYear$Naive[12:17])/sum(NaiveByYear$Naive)),2),
round((sum(NaiveByYear$Naive[12:19])/sum(NaiveByYear$Naive)),2),
round((sum(NaiveByYear$Naive[11:30+1])/sum(NaiveByYear$Naive)),2),
round((sum(NaiveByYear$Naive[20:26])/sum(NaiveByYear$Naive)),2),
round((sum(NaiveByYear$Naive[19:30+1])/sum(NaiveByYear$Naive)),2),
...
yr_range<-c("2-5","5-11","5-16","5-18","5-25","5-35","5-46","10-29","11-16","11-18","11-30","19-25","19-30","19-35","19-46","0-29","0-44",">8")
...
costtable20<-read.csv("tables/cost_benefit_20.csv",header=T)
costtable50<-read.csv("tables/cost_benefit_50.csv",header=T)
costtable74<-read.csv("tables/cost_benefit_74.csv",header=T)
costtable20_no<-read.csv("tables/cost_benefit_no_20.csv",header=T)
costtable50_no<-read.csv("tables/cost_benefit_no_50.csv",header=T)
costtable1877_no<-read.csv("tables/cost_benefit_no_1877.csv",header=T)
costtable_50pc<-read.csv("tables/cost_benefit_50_percent_vacc_35.csv",header=T)


vacp_rv<-read.csv("tables/rv.csv",header=T)

vacp<-read.csv("tables/vacc_predictions.csv",header=T)
...
\begin{figure}
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/case_age_dist.pdf}
     \end{center}
...
\begin{sidewaysfigure}
\begin{center}
     \includegraphics[width=0.8\textwidth]{figures/case_ordert.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
     \includegraphics[width=0.8\textwidth]{figures/percap_ordert.pdf}
\end{center}
...
\begin{figure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/Cases_regmodel.pdf}
\end{center}
...
\begin{figure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/Cases_regmodel_prediction.pdf}
\end{center}
...
\begin{figure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/Cases_regmodel_resid.pdf}
\end{center}
...
@
\caption{Significance of different predictor variables for measles risk factors from 2007-2014, estimated using \autoref{eq:reg} and a quasipoisson error structure. Pr($>$F) is p-value, the statistical significance of the result.}
\label{table:anova}
\end{center}
\end{table}
...
\begin{sidewaysfigure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/vacc_age_2007_2014.pdf}
\end{center}
...
\begin{figure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/vacc_age_2007_2014_close.pdf}
\end{center}
\caption{Age of vaccination of vaccinated measles cases, 2007--12~June~2014 in details showing children $<100$ months. Recommended vaccination age in months is shown for the measles, mumps, rubella (MMR) dose 1 (15 months) and MMR dose 2 (4 years).}
\label{fig:vaccstat_close}
\end{figure}

\begin{figure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dob_vacc_unvac.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dob_vacc_dose.pdf}
\end{center}
\caption{The year of birth for those measles cases vaccinated with measles, mumps, and rubella (MMR) dose 1 is shown in the upper panel, MMR dose 2 the lower panel.}
\label{fig:yearandvacf}
\end{sidewaysfigure}

\begin{sidewaysfigure}
\begin{center}
     \includegraphics[width=0.9\textwidth]{figures/naive_allPop_yob.pdf}
\end{center}
...
latex(table_nir_mmr, file="", table.env=FALSE,rowname=NULL)
@
\caption{Summary of the proportion of 2006--2014 birth cohorts vaccinated against measles-mumps-rubella (MMR) dose 1 and MMR2 across area units. Area units are aggregations of meshblock data. See \autoref{fig:fig12006}, \autoref{fig:fig22006}, \autoref{fig:fig12007}, \autoref{fig:fig22007}, \autoref{fig:fig12008}, \autoref{fig:fig22008}, \autoref{fig:fig12009}, \autoref{fig:fig22009}, \autoref{fig:fig12010}, \autoref{fig:fig22010}, \autoref{fig:fig12011} and \autoref{fig:fig12012}.}
\label{table:tab_mmr_nir}
\end{table}

\begin{sidewaysfigure}
   \begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2006.pdf}
    \end{center}
...
\begin{sidewaysfigure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_2006.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2007.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_2007.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2008.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_2008.pdf}
 \end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2009.pdf}
 \end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_2009.pdf}
 \end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2010.pdf}
\end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_2010.pdf}
 \end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2011.pdf}
 \end{center}
...
\begin{sidewaysfigure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_2012.pdf}
 \end{center}
...
\begin{figure}
   \begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2006.pdf}
    \end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Auk_2006.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Auk_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2008.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Auk_2008.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Auk_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2010.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Auk_2010.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2011.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Auk_2012.pdf}
 \end{center}
...
\begin{figure}
   \begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2006.pdf}
    \end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Well_2006.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Well_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2008.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Well_2008.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Well_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2010.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_Well_2010.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2011.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_Well_2012.pdf}
 \end{center}
...
\begin{figure}
   \begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2006.pdf}
    \end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_ChCh_2006.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_ChCh_2007.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2008.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_ChCh_2008.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_ChCh_2009.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2010.pdf}
\end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR2_NIR_ChCh_2010.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2011.pdf}
 \end{center}
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_ChCh_2012.pdf}
 \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{nir_maps/nir_census_MMR1_NIR_cor.pdf}
     \end{center}
...
\subsection{Estimated immunity rates for $<2005$ birth cohorts}

Maps for the immunity among the area units for those ages not included on the NIR database were created based on 2013 vaccination coverage data for those born between 2001--2013 (assuming immunisation coverage at 24 months for those born in 2001--2005, Tomasz Kiedrzynski, personal comm.) and serosurvey estimates of immunity (born in 2000 or earlier) among different age classes provided by the Ministry. Vaccine efficacy for 0--4 year olds was not necessary for these analyses. For those $\leq$ 13 years 99\% vaccine efficacy was assumed (\autoref{table:sero}). For $\geq$ 14 year olds serological data were used and equivocal serological results were considered non-immune (\autoref{table:sero}). Again there was heterogeneity at the local level (\autoref{fig:sero_imm_map}) with pockets of the nation with lower coverage than other areas (\autoref{fig:sero_imm_a}, \autoref{fig:sero_imm_w}, and \autoref{fig:sero_imm_c}). Given immunity levels were based on national data this heterogeneity is due to population structure.

\begin{sidewaysfigure}
     \begin{center}
     \includegraphics[width=1.05\textwidth]{census_immunity_age.pdf}
     \end{center}
     \caption{The estimated immunity levels in 2013 at the area unit level using the immunity data at the national level. The estimated immunity data are presented in \autoref{table:sero}. See \autoref{fig:sero_imm_a}, \autoref{fig:sero_imm_w}, \autoref{fig:sero_imm_c}  for city maps and \autoref{fig:sero_imm_hist} for the distribution of immunity levels.}
     \label{fig:sero_imm_map}
\end{sidewaysfigure}

\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{census_immunity_age_Auk.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{census_immunity_age_Well.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{census_immunity_age_ChCh.pdf}
     \end{center}
...
@
\caption{Non-New Zealander travel and immigration numbers by country of origin for 0--19 year olds, 2013.}
...
@
\caption{Risk of measles importation in cases per million travellers per year to New Zealand in 2013, estimated by 0--19 year old international and all New Zealander traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:nzrisk12} and \autoref{table:imrisk12}.}
\label{table:risk12}
\end{table}
...
@
\caption{Indicator of risk of measles importation in cases per million travellers per year to New Zealand due to New Zealander travel in 2013, estimated by all traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:imrisk12} and \autoref{table:risk12}.}
...
@
\caption{Indicator of risk of measles importation in cases per million travellers per year to New Zealand due to non-New Zealander 0--19 year old travel and immigration in 2013, estimated by traveller numbers multiplied by measles incidence in the source or destination location, see also \autoref{table:nzrisk12} and \autoref{table:risk12}.}
...
Globally, analyses of 2013 data, the most complete and recent year of data available, suggest measles incidence is highest  and vaccination coverage is lowest in less developed nations (\autoref{table:incidence12} and \autoref{table:cover12}, and \autoref{fig:incidence12} and \autoref{fig:cover12}). Travel of 0--19 year old non-New Zealanders (whether for work, pleasure, etc.) is dominated by people from Australia (\autoref{table:imtravel12} and \autoref{fig:immigration12}). A number of other countries then follow, including United Kingdom, China, Japan and the United States. Travel to Australia is also the most common travel destination (\autoref{table:nztravel12} and \autoref{fig:nztravel12}). Together, these mean that the greatest travel location for New Zealanders and country immigrants and travellers come from is Australia (\autoref{table:immigration12} and \autoref{fig:immigration12}). However, many South Pacific nations also appear in the most visited country list by New Zealanders. Though the precise interactions between these different risk factors are unknown, the most simple, a product of measles incidence in 2013 and traveller numbers in 2013, suggest that those countries with greatest international travel, Australia, UK, China and South East Asian countries, pose the greatest risk to New Zealand for measles importation. Though immigration and travel from some Asian countries is lower, some Asian countries also pose a high risk of measles importation to New Zealand. Note that South Pacific islands do not appear to pose a greater risk, despite large numbers of travellers of New Zealand origin, perhaps because of low measles incidence in these nations. These data are shown in \autoref{table:risk12}. The breakdown of risk of measles importation by New Zealander and non-New Zealander travellers is in \autoref{table:nzrisk12}, \autoref{table:imrisk12}, \autoref{fig:nzrisk12} and \autoref{fig:imrisk12}. The measles incidence and vaccination coverage for those countries identified as posing the greatest risk for measles importation to New Zealand are in \autoref{table:toprisk}. 
...
\end{center}
\caption{Risk map for measles importation in cases per million travellers per year, based on measles incidence and non-New Zealander 0--19 year old and all ages of New Zealander travel figures, 2013. Dark grey indicates no data were available.}
\label{fig:risk12}
\end{sidewaysfigure}
...
\end{center}
\caption{Risk map for measles importation in cases per million travellers per year from New Zealander international travel, based on measles incidence and travel figures, 2013. Dark grey indicates no data were available.}
\label{fig:nzrisk12}
\end{sidewaysfigure}
...
\end{center}
\caption{Risk map for measles importation in cases per million travellers per year from 0--19 year old non-New Zealander international travel and immigration and measles incidence, 2013. Dark grey indicates no data were available.}
\label{fig:imrisk12}
\end{sidewaysfigure}
...
A published model of the dynamics of measles infections in New Zealand has been used in earlier studies to evaluate the vaccination strategy in New Zealand of MMR1 at 15 months and MMR2 before 5 years \citep{roberts0,roberts4,tobias98}. The results showed that achieving coverage of greater than 90\% at both vaccination opportunities is necessary if future epidemics of measles are to be prevented.
...
In order to prevent future epidemics, it is necessary that $\Rr_V<1$. Hence, the proportion of the na\"{i}ve population that must be effectively vaccinated through additional vaccination to prevent future outbreaks, $\Pv$ is
...
\item The reproduction number for measles in the partially immune New Zealand population is often close to and exceeding one, suggesting increased population level immunity is required to ensure prevention of measles persistence following importation.
 \item The proportion of the currently na\"{\i}ve population requiring additional and effective vaccination in New Zealand required to push $R_v$ below one ranges from \Sexpr{vacc_reqd_dhb_min}\% to \Sexpr{vacc_reqd_dhb_max}\% among DHBs, and is \Sexpr{vacc_reqd_percent}\% at the national level (approximately \Sexpr{vacc_reqd_count} vaccinations).
\end{itemize}
...
@
\caption{Estimated costs (NZ\$) for measles public health management in New Zealand, 1 January -- 9 March 2014. PHN: public health nurse; PHA: public health assistant; IMT: incident management team; SSO: safety/security officer (SSO).}
\label{table:direct}
\end{table}
...
@
\caption{Reported direct hospitalisation costs in NZ\$, with the number of cases (Cases), length of hospital stay (Days), total cost (Cost, NZ\$), cost per case (NZ\$) and cost per day (NZ\$) for patients with measles as the primary diagnosis, 2000--2014}
\label{table:hosp}
 \centering
 \begin{tablenotes}
      \small
      \item As of 11 July 2014. NC: not calculated ($<$ 1 day).
    \end{tablenotes}
\end{table}
...
@
\caption{Frequency of measles cases and number and proportion admitted to hospital by age group, 16 December 2013 -- 19 June 2014. Data from ESR.}
\label{table:freq}
\end{table}
...
\begin{figure}
\begin{center}
     \includegraphics[width=0.8\textwidth]{figures/p.pdf}
\end{center}
...
To estimate the benefits from additional immunity through vaccination, as estimated from the above modelling section (\autoref{section:epidemic_modelling}), we carried out a number of analyses. Primarily, we used \autoref{eq:prop} to estimate the proportion of the na\"{\i}ve population in each DHB (\autoref{table:attack}) and at the national level requiring immunisation to reduce the $R_v$ to <1 (\autoref{section:epidemic_modelling}). We assume that the number of cases prevented by this is proportional to the outbreak size that might be expected. We estimated the expected outbreak sizes that might be prevented by two means. The first used was based on the annual mean number of measles cases from 2009 onward \autoref{table:vacp}. This number of measles cases assumes similar public health measures and introduction rates for the next ten years. The second approach was to use the number of measles cases predicted by epidemiological theory assuming no additional vaccination beyond the current levels was provided and no public health activities were performed (\autoref{table:attack} in \autoref{section:epidemic_modelling}). For this scenario in which the epidemiological models assume no intervention, we reduced the costs saved per case by removing all costs associated with public health activities (e.g. exclusion of contacts from schools and associated salary losses) but kept costs associated with measles related illness (e.g. general practicioner visits and hospitalisation costs). There is also a continued risk of introduction and despite $R_v$ being <1, smaller outbreaks may occur. Thus, we simulated expected outbreak sizes with $R_v$ < 1. We use these values of numbers of predicted cases prevented and numbers expected despite additional immunisation to calculate the savings. 

The cost figures above are used to estimate the savings for immunising additional populations, using estimated per case costs saved. The costs of the catch up immunisation schemes are estimated to be two different values, to determine how sensitive the benefit--cost ratio (B/C) was to differing immunisation costs. Values of \$20 and \$50 per immunisation are used, based on US literature, however, this assumes all costs are included with the vaccine delivery. Therefore, we also estimate approximate cost an effective immunisation (including the delivery costs) would need to be before the immunisation costs exceed the benefits. Thus the expected measles-related costs due to constant introduction of measles despite increased population immunity and the costs of the immunisation schemes are used to estimate the costs of additional vaccine programs for the B/C analysis. The financial savings from reduced measles cases are used to estimate the benefits for the additional immunisation programs. The costs for the catch up immunisation programs are presumed to be over a single year. The benefits for the cases saved are presumed to be over a ten year period. Because benefits are assessed over a 10--year time period, a discounting rate of 3\% discount per year for the costs saved was used, as is common for healthcare discounting \citep{honeycutt6}. Measles introductions are expected to be annually occurring events, though the costs per year for each event was also discounted. Thus, the B/C ratio was:
...
@
\caption{DHB and national level catch up immunisation rates and estimated outbreak sizes post catch-up immunisation. \textit{Population}: DHB Population, Statistics NZ 2013;    
\textit{Na\"{\i}ve}:	DHB na\"{\i}ve population $\left(x_0\times\text{Population}\right)$;		
\textit{Attack}: Number infected in DHB based on the annual mean number of reported measles cases in each DHB from 2009 onward;
\textit{Vaccination}: Number to be immunised in DHB to reduce $\Rr_V$ below one  $\left(\left(x_0-1/\Ro\right)\times\text{Population}\right)$; \textit{PC}: the proportion of the currently na\"{i}ve population requiring immunisation; \textit{Na\"{i}ve.post.vaccination}: the na\"{i}ve population following catch up immunisation; with \textit{Median.outbreak} and \textit{Mean.outbreak}: the expected median and mean outbreak size post--immunisation catch up from 1000 simulations of a stochastic model.}
...
\begin{figure}
     \centering
     \includegraphics[width=1\textwidth]{figures/sim1.pdf}
...
<<results=tex,echo=FALSE>>=
colnames(costtable20)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable20[,c(2:10,12)] <- round(costtable20[,c(2:10,12)])
costtable20[,11] <- round(costtable20[,11],2)
latex(costtable20, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with 20 dollars per immunised person assuming the cases prevented through additional vaccination each year are equal to the mean annual number of cases per DHB since 2009 (see \autoref{table:vacp}). \textit{Vacc} is numbers to immunise (see \autoref{table:vacp}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$2,624.57 per case); \textit{Manage saved} is management costs saved (\$1,765 per case), plus \$20 GP costs; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}	is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost20}
\end{sidewaystable}

\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable50)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable50[,c(2:10,12)] <- round(costtable50[,c(2:10,12)])
costtable50[,11] <- round(costtable50[,11],2)
latex(costtable50, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with 50 dollars per immunised person assuming the cases prevented through additional vaccination each year are equal to the mean annual number of cases per DHB since 2009 (see \autoref{table:vacp}). \textit{Vacc} is numbers to immunise (see \autoref{table:vacp}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$2,624.57 per case); \textit{Manage saved} is management costs saved (\$1,765 per case), plus \$20 GP costs; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}	is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost50}
\end{sidewaystable}

\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable74)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable74[,c(2:10,12)] <- round(costtable74[,c(2:10,12)])
costtable74[,11] <- round(costtable74[,11],2)
latex(costtable74, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with \$\Sexpr{break_even_vacc_cost} dollars per immunised person assuming the cases prevented through additional vaccination each year are equal to the mean annual number of cases per DHB since 2009 (see \autoref{table:vacp}). This is the break-even cost per immunised person (B/C = 1) nationally. \textit{Vacc} is numbers to immunise (see \autoref{table:vacp}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$2,624.57 per case); \textit{Manage saved} is management costs saved (\$1,765 per case), plus \$20 GP costs; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}  is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost74}
\end{sidewaystable}
...
\begin{figure}
     ...
     \caption{The relationship between the benefit--cost ratio and the cost of immunising individuals in the New Zealand population. The mean number of measles cases per years since 2009 was used the expected number of cases prevented from the additional vaccination (\autoref{table:cost20},\autoref{table:cost50},\autoref{table:cost74}).}
     \label{fig:bc}
\end{figure}

\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable20_no)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable20_no[,c(2:10,12)] <- round(costtable20_no[,c(2:10,12)])
costtable20_no[,11] <- round(costtable20_no[,11],2)
latex(costtable20_no, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with 20 dollars per immunised person assuming the cases prevented through additional vaccination each year are equal those based on epidemiological theory with no measles management costs (see \autoref{table:attack}). \textit{Vacc} is numbers to immunise (see \autoref{table:attack}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$0 per case)\footnote{Here we assume no exclusions}; \textit{Manage saved} is management costs saved (\$0 per case), plus \$20 GP costs\footnote{Here we assume no public health management costs per measles case except GP visits}; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}  is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost20_no}
\end{sidewaystable}

\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable50_no)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable50_no[,c(2:10,12)] <- round(costtable50_no[,c(2:10,12)])
costtable50_no[,11] <- round(costtable50_no[,11],2)
latex(costtable50_no, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with 50 dollars per immunised person assuming the cases prevented through additional vaccination each year are equal those based on epidemiological theory with no measles management costs (see \autoref{table:attack}). \textit{Vacc} is numbers to immunise (see \autoref{table:attack}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$0 per case)\footnote{Here we assume no exclusions}; \textit{Manage saved} is management costs saved (\$0 per case), plus \$20 GP costs\footnote{Here we assume no public health management costs per measles case except GP visits}; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}  is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost50_no}
\end{sidewaystable}

\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable1877_no)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable1877_no[,c(2:10,12)] <- round(costtable1877_no[,c(2:10,12)])
costtable1877_no[,11] <- round(costtable1877_no[,11],2)
latex(costtable1877_no, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with \$\Sexpr{break_even_vacc_cost_no} dollars per immunised person assuming the cases prevented through additional vaccination each year are equal those based on epidemiological theory with no measles management costs (see \autoref{table:attack}). \textit{Vacc} is numbers to immunise (see \autoref{table:attack}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$0 per case)\footnote{Here we assume no exclusions}; \textit{Manage saved} is management costs saved (\$0 per case), plus \$20 GP costs\footnote{Here we assume no public health management costs per measles case except GP visits}; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}  is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}  is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost1877_no}
\end{sidewaystable}


\begin{sidewaystable}\footnotesize
<<results=tex,echo=FALSE>>=
colnames(costtable_50pc)<-c("DHB", "Vacc", "Vacc costs", "Wages saved", "Manage saved", "Hosp saved", "Contacts","Costs saved", "Outbreak", "OB costs", "B/C","NPV")
costtable_50pc[,c(2:10,12)] <- round(costtable_50pc[,c(2:10,12)])
costtable_50pc[,11] <- round(costtable_50pc[,11],2)
latex(costtable_50pc, file="", table.env=FALSE,rowname=NULL)
@
\caption{Benefit--cost analyses with \$\Sexpr{break_even_vacc_cost50} dollars per immunised person assuming the cases prevented through additional vaccination each year are equal to the mean annual number of cases per DHB since 2009 (see \autoref{table:vacp}). This is the break-even cost per immunised person (B/C = 1) nationally if 50\% of the currently na\"{\i}ve population were vaccinated. \textit{Vacc} is numbers to immunise if 50\% of the currently na\"{\i}ve population were vaccinated (see \autoref{table:vacp}); \textit{Vacc costs} is cost for the catch up immunisation programme; \textit{Wages saved} is wages of care givers and cases saved (\$839 per case); \textit{Contacts} is costs saved through contact exclusion (\$2,624.57 per case); \textit{Manage saved} is management costs saved (\$1,765 per case), plus \$20 GP costs; \textit{Hosp saved} is the hospitalisation costs saved (\$1,877 per case); \textit{Costs save}  is the discounted costs saved; \textit{Outbreak}  is the predicted median outbreak size despite $R_v < 1$ due to measles importation from 1000 simulations\footnote{Note the median outbreak sizes are what is expected, while the mean takes into account rare, but large outbreaks, see \autoref{fig:sim} and \autoref{fig:sim1}.}; \textit{OB costs}	is costs expected due to continued measles importations based on 10 introductions of measles, one per year, but costs discounted on the same discounted rate; \textit{B/C} is the benefit--cost ratio; \textit{NPV} is the net present value, defined as the sum of the present values of costs and benefits of the vaccination programme.}
\label{table:cost50pc}
\end{sidewaystable}
...
The model estimates for each DHB for measles outbreaks assuming no public health interventions and homogeneous mixing, with the immunisation percentages and the expected outbreak size based on following additional immunisation is shown in \autoref{table:attack}. 

The benefit--cost results for the scenario where the annual mean number of measles cases since 2009 are expected to be prevented due to increased vaccination are in \autoref{table:cost20} and \autoref{table:cost50}, for two different vaccine costs. The results in the two tables show the benefits of immunisation are always substantially greater than the costs of the increased supplementary immunisation (\autoref{table:cost20} and \autoref{table:cost50}). The expected threshold value when the immunisation costs per immunised person break even in the B/C analysis (B/C is 1) are in \autoref{table:cost74}. At this point (national B/C = 1), the additional vaccination becomes costly to some DHBs. The cost we estimated was \$\Sexpr{break_even_vacc_cost} per immunised person before vaccination was not cost effective over the 10 year period modelled. The relationship between the B/C ratio and the cost per immunised person at the national level is in \autoref{fig:bc}.

The benefit--cost results for the scenario where the predicted number of measles cases based on epidemiological theory are expected to be prevented due to increased vaccination, in the absence of additional public measures, such as contact exclusion, are in \autoref{table:cost20_no} and \autoref{table:cost50_no}, for two different vaccine costs. The results in the two tables show the benefits of immunisation are always substantially greater than the costs of the increased supplementary immunisation (\autoref{table:cost20} and \autoref{table:cost50}). The cost of immunisation per person under this scenario, without public health measures and assuming homogeneous population mixing, was required to reach \$\Sexpr{break_even_vacc_cost_no} before the costs of additional immunisation exceeded the benefits (\autoref{table:cost1877_no}).

The last benefit--cost analysis scenario performed was one in which more than the minimum proportion of na\"{\i}ve were immunised. In this scenario we did not use the \Sexpr{vacc_reqd_percent}\% of currently na\"{\i}ve population necessary to reach adequate national immunity levels (\autoref{table:coh_vac}), but included additional vaccination so that 50$\%$ of the currently na\"{\i}ve population was immunised, \autoref{table:cost50pc}. In this scenario the additional vaccination reduces the B/C ratio, and the additional vaccines for the extra immunisation mean that the cost per immunised person before the programme becomes costly reduces to \$\Sexpr{break_even_vacc_cost50}  per immunised person \autoref{table:cost50pc}.

In concordance with current measles knowledge, strategies that target the very young (<1 year old) are not effective, and our analyses of the vaccinated cases suggests a substantial proportion of vaccinated cases (\autoref{fig:ageandvac}) were vaccinated with a single vaccine around 1 year of age (\autoref{fig:vaccstat} and \autoref{fig:vaccstat_close}). Furthermore, it appears possible to reach the appropriate figure (\Sexpr{vacc_reqd_percent}\% of currently na\"{\i}ve) by vaccinating all the currently na\"{\i}ve older population groups with lower coverage, in particular school age children and young adults. However, there are a number of different potential vaccination schemes that will lead to sufficient coverage and these are given in \autoref{table:coh_vac}. The appropriate cover could be reached by catch up vaccination of all school aged children, i.e. vaccinating all currently na\"{\i}ve would immunise (\Sexpr{vacc_reqd_percent}\% of currently na\"{\i}ve population, reaching the necessary national immunity levels (\autoref{table:coh_vac}).

\begin{table}[hbtp] %\scriptsize
\captionsetup{width=1\textwidth}
<<results=tex,echo=FALSE>>=
colnames(propvCohort)<-c("Age range (yrs)","Proportion naive")
latex(propvCohort, file="", table.env=FALSE,rowname=NULL)
@
\caption{Potential catch up vaccination schemes showing the proportion of the currently na\"{\i}ve New Zealand population that would be immunised if all na\"{\i}ve people in each age range were immunised. The estimated additional proportion of the population requiring immunisation is \Sexpr{vacc_reqd_percent}\%.}
\label{table:coh_vac}
\end{table}

\subsection{Benefit--cost analyses discussion}

Our estimates of the costs of the measles outbreaks in New Zealand suggest that measles management costs per case in New Zealand is high. We used the mean values per case in the absence of alternative data and used all available data. The exact costs will vary, however, our benefit--cost analyses suggest that in all cases catch up immunisation schemes will be financially beneficial for New Zealand (see \autoref{table:cost20} and \autoref{table:cost50}). Additional vaccination beyond the \Sexpr{vacc_reqd_percent}\% of currently na\"{\i}ve population we estimate are required to be vaccinated for $R_0 < 1$ decrease the cost effectiveness of an immunisation program. This is because large outbreaks are avoided once $R_0 < 1$. In the scenario we modelled with 50\% of the currently na\"{\i}ve population immunised, costs exceed benefits at \$\Sexpr{break_even_vacc_cost50} per immunised person \autoref{table:cost50pc}. There may be other benefits we have not accounted for in our analyses for additional vaccination. These additional benefits may include reducing the possibility of spatial heterogeneity and future benefits beyond the ten year period we modelled. Our analyses do suggest that in the absence of control measures the benefits of vaccination are substantial if measles were to spread through the na\"{\i}ve population as predicted by epidemiological theory (\autoref{table:cost20_no}, \autoref{table:cost50_no}, and \autoref{table:cost1877_no}).
...
Our estimates for the benefit--cost ratio of catch up immunisation are similar to those in other countries. Estimates in Korea suggest catch up immunisation schemes have a benefit--cost ratio of just over one \citep{bae13}. If we presume the $R_0$ estimated for measles in New Zealand is representative and following \Sexpr{vacc_reqd_percent}\% immunisation of the na\"{\i}ve population, measles will be unable to persist ($R_v$ < 1), our B/C ratio is greater than 1. At the national level our B/C ratio is > 1 for either \$50 or \$20 effective immunisation costs. However, the B/C ratios do vary substantially by DHB, with those with smaller populations least cost effective for immunisation programmes. However, the estimated B/C ratio for MMR has been estimated to be between 10.2 and 54.2, much greater than one (see \autoref{table:zhou2}) \citep{zhou4}. As stated above, these authors also consider the costs and benefits of mumps and rubella, so making direct comparison difficult without reanalysis. We did not include the B/C ratio of the complete MMR immunisation programme, though presumably if this was included our B/C ratio may approach the ratios reported in those analyses that do \citep{zhou4}. Our results are relatively insensitive to changes in immunisation cost, requiring high costs (>\$\Sexpr{break_even_vacc_cost} per immunised person at the national level) before immunisation is predicted to not be beneficial. Similarly, longer discounting periods or 5\% discounting did not affect the B/C ratio substantially (not shown). A spreadsheet is provided for the Ministry to work with.
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/case_age_nzdep.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/case_eth_nzdep.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/case_age_eth.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/percap_nzdep_age.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/percap_eth_nzdep.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/percap_eth_age.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb1.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb2.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb3.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb4.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb5.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb6.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb7.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb8.pdf}
     \end{center}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/dhb9.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb10.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb11.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb12.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb13.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb14.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb15.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb16.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb17.pdf}
     \end{center}
...
\begin{figure}[H]
   \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb18.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb19.pdf}
     \end{center}
...
\begin{figure}[H]
     \begin{center}
     \includegraphics[width=0.9\textwidth]{figures/dhb20.pdf}
     \end{center}
